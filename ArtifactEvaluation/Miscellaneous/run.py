...
def AutoQ_permutation(root, str1, semaphore):
    with semaphore:
        ...
        p = subprocess.run(f'VATA_PATH=/home/alan23273850/libvata/build/cli/vata timeout 3600 ../../build/cli/vata {root}/bug.qasm {root}/spec.qasm {root}/post1.aut {root}/post2.aut', shell=True, capture_output=True, executable='/bin/bash')
        ret = p.returncode
        if ret == 0:
...
def AutoQ_composition(root, str2, semaphore):
    with semaphore:
        ...
        p = subprocess.run(f'VATA_PATH=/home/alan23273850/libvata/build/cli/vata timeout 3600 ./composition_find_bug {root}/bug.qasm {root}/spec.qasm {root}/post1.aut {root}/post2.aut', shell=True, capture_output=True, executable='/bin/bash')
        ret = p.returncode
        if ret == 0:
...
def Feynman(root, str3, semaphore):
    with semaphore:
        p = subprocess.run(f'timeout 3600 /home/alan23273850/feynman/feynver {root}/spec.qc {root}/bug.qc', shell=True, capture_output=True, executable='/bin/bash')
        ret = p.returncode
        if ret == 124:
            ...
            str3.value = tmp.split(' (took ')[1].split(')')[0] + ' & ' + tmp.split(' (took ')[0]
...
for root, dirnames, filenames in sorted(os.walk('.')):
    if len(dirnames) == 0 and 'post.aut' in filenames:
        ...
        for func in (AutoQ_permutation,):# AutoQ_composition, Feynman):
